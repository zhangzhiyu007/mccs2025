        ...
        msg_type_t msg_type)
{
    ...
    if (msg_type == MSG_INDICATION)
    {
        ...
        else if (function == _FC_WRITE_MULTIPLE_COILS || function
        ...
        msg_type_t msg_type)
{
    ...
    {
        /* MSG_CONFIRMATION */
        if (function <= _FC_READ_INPUT_REGISTERS || function
                == _FC_REPORT_SLAVE_ID || function
...
static int receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    while (length_to_read != 0)
    {
        ...
        if (rc == -1)
        {
            ...
            if ((ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) && (errno
                    == ECONNRESET || errno == ECONNREFUSED || errno == EBADF))
            {
                int saved_errno = errno;
                modbus_close(ctx);
                modbus_connect(ctx);
        ...
        int rsp_length)
{
    ...
    /* Check length */
    if (rsp_length == rsp_length_computed || rsp_length_computed
        ...
        modbus_mapping_t *mb_mapping)
{
    ...
    case _FC_READ_DISCRETE_INPUTS://
    {
        ...
        break;
    case _FC_READ_HOLDING_REGISTERS://3
    {
        int nb = (req[offset + 3] << 8) + req[offset + 4];
        ...
        else
        {
            //读取数据库实时数据
        	int nNum=0;
            RealData realData[512];
            bool success=MemDb::GetRealData(realData,512,address,nb);
            if (!success)
            {
                rsp_length = response_exception(ctx, &sft,
            ...
            else
            {
                ...
                for (i = address; i < address + nb; i++)
                {
                	if (realData[nNum].pv.type == RealData::DB)
					{
                		data=(int)realData[nNum].pv.data.dbValue;
					}
                	else
                	{
                		data=(int)realData[nNum].pv.data.iValue;
                	}

                    rsp[rsp_length++] = (data >> 8)&0xFF;
                    rsp[rsp_length++] = (data >> 0)&0xFF;
                    nNum++;
                }
            }
        }
        ...
        break;
    case _FC_READ_INPUT_REGISTERS://4
    {
    ...
    case _FC_WRITE_SINGLE_REGISTER:
        ...
        else
        {
			...
			//写入数据库
			bool ret = MemDb::SetRealDataUnknown(address, data,true);
			if (!ret)
			{
				rsp_length = response_exception(ctx, &sft,
						MODBUS_EXCEPTION_SLAVE_OR_SERVER_BUSY, rsp);
    ...
    case _FC_WRITE_MULTIPLE_REGISTERS:
    {
        ...
        else
        {
            ...
            {
                /* 6 and 7 = first value */
            	data = (req[offset + j] << 8) + req[offset + j + 1];
            	ret = MemDb::SetRealDataUnknown(i, data,true);
            }
    ...
    case _FC_WRITE_AND_READ_REGISTERS:
    {
        ...
        if ((address + nb) > mb_mapping->nb_registers || (address_write
                + nb_write) > mb_mapping->nb_registers)
        {
            if (ctx->debug)
            {
                fprintf(
...
//
//            PtrArray array;
//            DATA_REAL* pDataReal=NULL;
//            int nNum=0;
//            /* Write first.
//             10 and 11 are the offset of the first values to write */
//            for (i = address_write, j = 10; i < address_write + nb_write; i++, j
//                    += 2)
//            {
//                pDataReal=new DATA_REAL;
//                pDataReal->pv = (req[offset + j] << 8) + req[offset + j + 1];
//                pDataReal->sn = i + 1;
//                pDataReal->write = 1;
//                pDataReal->read = 0;
//                array.push_back(pDataReal);
//            }
//            //保存到数据库
//            nNum = RealDB::setRealData(array);
//            RealDB::freeArray(array);
//
//            /* and read the data for the response */
//            int reg_temp[512] = { 0 };
//            //读取数据库实时数据
//            nNum = RealDB::getRealData(address, nb, reg_temp);
//            for (i = address; i < address + nb; i++)
//            {
//                rsp[rsp_length++] = (uint8_t)(reg_temp[i] >> 8);
//                rsp[rsp_length++] = reg_temp[i] & 0xFF;
//            }
        }
    }
        break;
        ...
        uint8_t *dest)
{
    ...
    req_length
            = ctx->backend->build_request_basis(ctx, function, addr, nb, req);
        ...
        uint16_t *dest)
{
    ...
    if (rc > 0)
    {
        ...
        for (i = 0; i < rc; i++)
        {
            ...
            dest[i] = (rsp[offset + 2 + (i << 1)] << 8) | rsp[offset + 3 + (i
                    << 1)];
        }
    }
...
int modbus_write_and_read_registers(modbus_t *ctx, int write_addr,
        int write_nb, const uint16_t *src, int read_addr, int read_nb,
        uint16_t *dest)
        ...
        int nb_registers, int nb_input_registers)
{
    ...
    {
        mb_mapping->tab_registers = (uint16_t *) malloc(nb_registers
                * sizeof(uint16_t));
    ...
    else
    {
        ...
        memset(mb_mapping->tab_input_registers, 0, nb_input_registers
                * sizeof(uint16_t));
    }
