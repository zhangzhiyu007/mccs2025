/*
 * Check.cpp
 *
 *   创建日期: 2014-1-10
 *   作          者: 马中华
 */

#include "Check.h"

const unsigned char Check::m_crcHigh[] = {  };

const unsigned char Check::m_crcLower[] = {  };

Check::Check()
{
	// TODO 构造函数

}

Check::~Check()
{
	// TODO 析构函数
}

unsigned short Check::Crc16(String data)
{
	unsigned char crcHi = 0xFF;
	unsigned char crcLo = 0xFF;
	int index;

	int len = data.size();
	int i=0;
	while (len--)
	{
		index = crcLo ^ (unsigned char)data[i];
		crcLo = (crcHi ^ Check::m_crcHigh[index]);
		crcHi = Check::m_crcLower[index];
		i++;
	}
	return (unsigned short) (crcHi << 8 | crcLo);
}

unsigned short Check::CheckSum(String data)
{
	unsigned short crc16 = 0;
	unsigned int i = 0;

	for (i = 0; i < data.size(); i++)
	{
		crc16 += data[i];
	}

	crc16 = crc16 - 0x7B;

	return (crc16 & 0xFF);
}
